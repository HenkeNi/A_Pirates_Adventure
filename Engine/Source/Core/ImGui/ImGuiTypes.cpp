...
namespace Hi_Engine
{
	...
	void ImGuiWindow::Update() 
	{
		...
		const auto& texture = ResourceHolder<Subtexture2D, SubtextureData>::GetInstance().GetResource({ "folder", 0, 0 });

		//ImTextureID textureID = static_cast<void*>(texture.GetTexture().GetData());
		ImTextureID textureID = reinterpret_cast<void*>(static_cast<intptr_t>(texture.GetTexture().GetID()));
