...
namespace Hi_Engine
{
	...
	RenderService::RenderService(std::weak_ptr<WindowService> windowService)
		: m_windowService{ windowService }	// : m_buffer{ new Vertex[Constants::maxVertexCount] }, m_currentVertex{ nullptr }
	{
	...
	void RenderService::Deserialize(const char* path)
	{
	...
	void RenderService::SetProjectionMatrix(const glm::mat4& proj)
	{
	...
	void RenderService::AddSpriteBatch(SpriteBatch&& batch)
	{
	...
	void RenderService::ProcessBatches()
	{
		BeginNewBatch();
	...
	void RenderService::EndFrame()
	{
		...
		//assert(m_windowService && "Window's nullptr");

		if (auto windowService = m_windowService.lock())
			windowService->SwapBuffers(); // call from render system?
	}

	void RenderService::Display()
	{
	...
	void RenderService::DrawSprite(const Sprite& sprite)
	{
	...
	void RenderService::DrawQuad(const QuadRenderData& data)
	{
	...
	void RenderService::SetShader(GLSLShader* shader)
	{
	...
	bool RenderService::IsTextureBound(unsigned texID, float& outTexIndex)
	{
	...
	void RenderService::ClearScreen()
	{
		glClearColor(0.2f, 0.3f, 0.3f, 1.0f);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	}

	void RenderService::SetupVertexArray()
	{
	...
	void RenderService::BeginNewBatch()
	{
