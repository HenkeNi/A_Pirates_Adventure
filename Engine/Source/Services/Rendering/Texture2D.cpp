...
namespace Hi_Engine
{
	...
	std::optional<Texture2D> Texture2D::FromJSON(const rapidjson::Value& json)
	{
		//if (!json.IsObject())
		//{
		//	return std::nullopt;
		...
		//Texture2D texture;
		//texture.m_name = json["id"].GetString();

		//std::string path = TextureDirectory + value["path"].GetString();
	...
	void Texture2D::Init(const std::string& name, const std::string& path, const std::string& filtering)
	{
		stbi_set_flip_vertically_on_load(true);
	...
	//{
		...
		//		auto subtexture = std::make_shared<Subtexture2D>(*this);
