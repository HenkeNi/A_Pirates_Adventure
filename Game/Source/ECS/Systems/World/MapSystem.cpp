...
std::shared_ptr<Hi_Engine::Subtexture2D> GetSubtexture(eTile type) // TODO: check neighbours?
{
...
void MapSystem::Receive(Message& message)
{
	assert(m_ecs && "[MapSystem::Receive] - ECS is not initialized!");
...
{
	assert(m_ecs && "[MapSystem::Update] - ECS is not initialized!");
...
void MapSystem::GenerateMapChunk(int xCoord, int yCoord)
{
	Entity entity = m_ecs->CreateEntityFromBlueprint("MapChunk", false);
	...
	if (!mapChunkComponent || !transformComponent)
		return;

	mapChunkComponent->Coordinates = { xCoord, yCoord };

	auto& position = transformComponent->CurrentPos;
	position.x = xCoord * (Constants::MapChunkLength * Tile::Size);
	position.y = yCoord * (Constants::MapChunkLength * Tile::Size);

	for (int i = 0; i < Constants::MapChunkSize; ++i)
	{
		int xCoordinate, yCoordinate;
		MapUtils::GetCoordinates(i, Constants::MapChunkLength, xCoordinate, yCoordinate);

		float tilePositionX = ((float)position.x / Tile::Size) + (float)xCoordinate;
		float tilePositionY = ((float)position.y / Tile::Size) + (float)yCoordinate;

		float noise = UtilityFunctions::GetTerrainNoise(tilePositionX, tilePositionY);

		Tile& tile = mapChunkComponent->Tiles[i];
		tile.Coordinates = { (unsigned)xCoordinate, (unsigned)yCoordinate };
		tile.Type = GetTileType(noise);
		tile.IsTraversable = tile.Type != eTile::DeepWater && tile.Type != eTile::Water;
	}

	ApplyTileTextures(entity);
	PostMaster::GetInstance().SendMessage({ eMessage::EntityCreated, entity });
}

void MapSystem::ApplyTileTextures(Entity entity) // Rename; texture map chunk?
{
	...
	for (int i = 0; i < mapChunkComponent->Tiles.size(); ++i)
	{
		...
		sprite.Subtexture = tile.Subtexture.get(); // FIX!!!!!!!!!!
...
void MapSystem::SetupMapEditor()
{
	...
	//Hi_Engine::EventDispatcher::GetInstance().SendEventInstantly<Hi_Engine::CreateImGuiWindowRequest>(imGuiWindow);
}

void MapSystem::RegenerateMap()
{
