...
{
	s_systemManager.Update(aDeltaTime); // or pass in all entities and let system update the ones they need...
}

void GameScene::LateUpdate(float aDeltaTime)
{
	s_systemManager.LateUpdate(aDeltaTime);
}
...
{
	s_systemManager.Init(&m_entityManager);
	...
	s_systemManager.Register(std::move(cameraSystem));
	s_systemManager.Register(std::move(mapRenderSystem));
	s_systemManager.Register(std::move(spriteRenderSystem)); 
